...
class CreateMstMembersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('mst_members', function (Blueprint $table) {
            ...
            $table->string('photo', 50)->comment('member\'s photo')->nullable();
            $table->date('DOB')->comment('member\'s date of birth');
            $table->string('email', 50)->unique('email')->comment('member\'s email id')->nullable();
            $table->string('address', 200)->comment('member\'s address')->nullable();
            $table->boolean('status')->comment('0 for inactive , 1 for active');
            $table->string('proof_name', 50)->comment('name of the proof provided by member')->nullable();
            $table->string('proof_photo', 50)->comment('photo of the proof')->nullable();
