...
<div class="wrapper animated fadeInDown">
    <div class="panel overflow-hidden">
        ...
        <form id="resetPasswordform" method="post" action="{{ route('post_reset_passord', ['token' => $token]) }}">
            <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
            <div class="box-body padding-md">

                <div class="form-group">
                    <p>{{ @trans('custom.make_new_password') }}</p>
                </div>
                
                <div class="form-group">
                    <input type="text" name="email" class="form-control input-lg" value="{{ $user->email }}" readonly/>
                </div>
                ...
                <button type="submit" class="btn btn-dark bg-light-green-500 padding-10 btn-block color-white"><i class="ion-log-in"></i> {{@trans('custom.btn_change_password')}}</button>
            </div>
        </form>
