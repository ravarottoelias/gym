...
<div class="table-responsive {!! (! $outstandings->isEmpty() ? 'panel-scroll' : '')  !!}">
    <table class="table table-hover">
        @forelse($outstandings as $outstanding)
            <tr>
                <td>
                    <a href="{{ action('ExpensesController@edit',['id' => $outstanding->id]) }}"><strong class="text-muted">{{ $outstanding->name }}</strong></a>
                </td>
                <td>
                    <a href="{{ action('ExpensesController@edit',['id' => $outstanding->id]) }}">  @money($outstanding->amount)</a>
                </td>
                <td>
